...
LOCAL_ENTRIES = [
    dict(
        ...
        observations=[
            ...
            "(con1=HDMI, con2=DP) ซึ่งเสถียร",
            "ปัญหา USB 3.0: macOS สร้างพอร์ต USB2 15 พอร์ต แต่ SuperSpeed = 0 เพราะ `UTBDefault.kext` "
            "เป็น placeholder ที่ไม่มีข้อมูลพอร์ต → ต้องสร้าง USB port map จริงด้วย USBToolBox",
            "เอกสารตั้งชื่อเครื่องเพียง **Dell (i7-7700 / Intel HD 630)** ไม่ได้ระบุรุ่น — "
            "ชื่อไฟล์ EFI และโฟลเดอร์ใน zip เป็น `EFI-Dell-Precision-3420` จึงคงชื่อตามไฟล์ EFI",
            "🔒 **ไฟล์นี้ถูก sanitize ก่อนเผยแพร่** — ล้าง `SystemSerialNumber`, `MLB`, `SystemUUID`, "
            "`ROM` ใน `config.plist*` ทุกไฟล์ใน zip (30 ไฟล์) โดยคง `SystemProductName = MacPro7,1` ไว้ · "
            "md5 ปัจจุบัน `d712d5aee660aed97d952033398ae9d7` → **ต้องสร้าง SMBIOS ของตัวเองก่อนใช้**",
            "ใน zip มีไฟล์สำรอง `config.plist.bak*` (ถึง bak33) และ `_kexts/_old-kexts/` ติดมาด้วย — "
            "เก็บตามต้นฉบับ",
